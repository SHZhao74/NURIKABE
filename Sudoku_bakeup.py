...
class PyGameBoard():
  ...
  def __init__(self, engine, windowSize, gridValues):
    ...
    board = pygame.image.load('55.png')
...
class Tile():
  ...
  def __init__(self, value, coords, gridLoc, size):
    ...
    self.__colorSquare.fill(pygame.color.THECOLORS['white'], None, pygame.BLEND_RGB_ADD)#white
...
class Sudoku:
  ...
  def startNewGame(self):
    ...
    print 'Done' 
    board = PyGameBoard(self, (500,500), gridValues)
  ...
  def lineToGrid(self, linePuzzle):
    assert (len(linePuzzle) == 25)
    grid = []
    for i in xrange(0, 25, 5):
      grid.append(linePuzzle[i:i+5])
    return grid 
